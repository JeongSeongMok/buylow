/*
 * 토스 브로커리지 모델 + 한국식 수수료 모델.
 *
 * - 기본 시장을 krx로(주식). 한국 개인 현금계좌 가정(AccountType.Cash, 레버리지 1).
 * - 수수료/세금은 market/krx.py·KIS 어댑터(KoreanFeeModel)와 같은 규칙(매수: 위탁수수료,
 *   매도: +증권거래세). 백테스트와 라이브가 동일 비용을 쓰게 해 결과 괴리를 줄인다.
 *   (KIS 어댑터의 KoreanFeeModel과 동일 로직 — 프로젝트 참조 없이 어댑터별 독립 빌드라 복제한다.)
 */
...
{
    /// <summary>한국 거래비용: 매수=위탁수수료, 매도=위탁수수료+증권거래세. KRW.</summary>
    public class KoreanFeeModel : FeeModel
    {
        ...
        public override OrderFee GetOrderFee(OrderFeeParameters parameters)
        {
            ...
            return new OrderFee(new CashAmount(Math.Round(fee), TossConstants.KrwCurrency));
        }
    }

    public class TossBrokerageModel : DefaultBrokerageModel
    {
        private static readonly IReadOnlyDictionary<SecurityType, string> KrxMarkets =
            new Dictionary<SecurityType, string> { { SecurityType.Equity, TossConstants.KrxMarket } }.ToReadOnlyDictionary();

        public TossBrokerageModel() : base(AccountType.Cash) { }
        ...
        public override bool CanSubmitOrder(Security security, Order order, out BrokerageMessageEvent message)
        {
            ...
            {
                message = new BrokerageMessageEvent(BrokerageMessageType.Warning, "NotSupported",
                    $"토스 어댑터가 지원하지 않는 주문유형: {order.Type}");
                return false;
            }
            message = null;
            return true;
