...
- **규칙 엔진**: EMA·MACD·RSI·모멘텀 등 조건을 `(EMA AND MACD) OR RSI`처럼 자유 조합 — 대시보드에선 **조건 그룹 빌더**(그룹 안=AND, 그룹끼리=OR)로 체크만 하면 됨. 한국 특화 Alpha(수급 추종·저PBR)
- **유니버스 선별**: 적재된 전 종목을 자동 스캔
- **백테스트**: 백그라운드 실행 + 실시간 로그/상태, 결과·이력 저장(SQLite)
- **리스크 관리(전역)**: 종목별 손절·익절·트레일링 스탑
- **로컬 대시보드**(3챕터): ① 전략 설정(조건식 + 리스크) · ② 백테스트(기간·유니버스 정해 실행 + 결과) · ③ 설정(키 + 전체 데이터 적재)
...
대시보드 흐름:
1. **③ 설정** — (필요 시) KRX 키 입력 → **[전체 종목 데이터 적재]** 버튼 한 번
2. **① 전략 설정** — 조건식 `(EMA AND MACD) OR RSI` + 리스크(손절·익절) 저장
3. **② 백테스트** — 기간·유니버스만 정해 실행 → 상태·로그·결과 확인
