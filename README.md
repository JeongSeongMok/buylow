...
- **계좌 모니터링** — 예수금·매수가능·보유종목(매수가/현재가/손익), 장중/장마감 상태, 매매 내역(KIS는 체결조회 기반, 토스는 자체 거래로그 기반, 10초마다 자동 갱신).
...
- **토스증권**은 모의투자 서버가 없어 실전 하나뿐이고, OAuth2 키(Client ID/Secret)만 입력하면 계좌는 자동으로 잡습니다(계좌번호·HTS ID 불필요). 체결통보 웹소켓이 없어 어댑터가 **주문 폴링**으로 체결을 확인합니다.
