...
- **규칙 엔진**: EMA·MACD·RSI·모멘텀·볼린저밴드 등 조건을 `(EMA AND MACD) OR RSI`처럼 자유 조합 — 대시보드에선 **조건 그룹 빌더**(그룹 안=AND, 그룹끼리=OR)로 체크만 하면 됨. 한국 특화 Alpha(수급 추종·저PBR)
...
- [x] 시그널 — EMA·MACD·RSI·모멘텀·**볼린저밴드(평균회귀+돌파전환)**
...
- [ ] 한국 특화 시그널 — **수급 추종·가치(저PBR)** 를 규칙 엔진 signal로
- [ ] 분봉 ETL → 단타·변동성 돌파 전략
- [ ] 파라미터 최적화(스윕) · 워크포워드
