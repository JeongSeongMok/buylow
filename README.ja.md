...
- **口座モニタリング** — 預り金・買付可能額・保有銘柄（買値/現在値/損益）、ザラ場/引け後の状態、取引履歴（KIS は約定照会ベース、トスは buylow 独自の取引ログベース、10 秒ごとに自動更新）。
...
- **トス証券**は模擬サーバーがなく実取引のみで、OAuth2 キー（Client ID/Secret）だけ入力すれば口座は自動取得します（口座番号・HTS ID 不要）。約定通知の WebSocket がないため、アダプタは**注文ポーリング**で約定を確認します。
