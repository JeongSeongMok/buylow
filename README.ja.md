...
Toss・AI の API キーは自分で用意します。解決順序: **環境変数**（例: `export BUYLOW_TOSS_APP_KEY=...`）
→ ローカルの `config.local.yaml` → どちらも無ければ**初回起動時にダッシュボードで入力**。キーは
ローカルの `config.local.yaml`（gitignore）に保存され、コミットされることはありません。
