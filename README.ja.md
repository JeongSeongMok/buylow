...
- ⚠️ **ライブには KIS アダプタ DLL のビルドが一度必要です**（バックテストには不要なためインストール手順では任意）。ビルドせずにトグルを ON にすると *「KIS アダプタがありません」* という案内が出ます — 上記 [セットアップ](#セットアップ) のアダプタビルド手順を実行してください。
...
以下が必要です — **.NET 10 SDK**（エンジンの実行）、**Python 3.11**（戦略の実行）、**uv**（Python 環境）、**git**。

<details open>
<summary><b>macOS</b></summary>

```bash
# 1) .NET 10 SDK（恒久化するには ~/.zshrc に export を追記）
...
```powershell
# 1) .NET 10 SDK · Python 3.11 · git · uv（インストール後、新しいターミナルを開いて PATH を反映）
...
# 3) ダッシュボードの起動（デフォルトポート 8420）
.venv\Scripts\python -m orchestrator.api
# 別のポートで開くには:  $env:BUYLOW_DASHBOARD_PORT=9000; .venv\Scripts\python -m orchestrator.api

# 4) （ライブ実取引用 — バックテストのみなら省略可）KIS アダプタのビルド
#    build-adapter.sh は bash 専用のため、Windows では下記の PowerShell コマンドで同様にビルドします。
dotnet build launcher\BuylowLauncher.csproj -c Release             # 先にランチャーをビルド（NuGet 復元）
dotnet build adapter\MyTrading.Kis\MyTrading.Kis.csproj -c Release # アダプタをビルド
Copy-Item adapter\MyTrading.Kis\bin\Release\net10.0\MyTrading.Kis.dll launcher\bin\Release\net10.0\
```

> ⚠️ **`uv` のインストール直後に `uv venv .venv` で「uv は cmdlet として認識されません…」というエラーが出る場合**、PATH が
> まだ更新されていません。**PowerShell のウィンドウを完全に閉じて新しく開き直し**、再実行してください。ウィンドウを閉じずに
> 現在のウィンドウで使うには `$env:Path = "$env:USERPROFILE\.local\bin;$env:Path"` で PATH に追加します。
> （`.NET`/`python` コマンドも同じ理由でインストール後は新しいターミナルが必要です。）
