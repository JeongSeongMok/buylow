...
**Feature-complete (backtest + live for KIS·Toss).** The whole backtest path — data, strategy, backtest, results — works end to end. Live trading is wired for **both KIS and Toss**; what remains is real-order e2e verification on a live/demo account (no automated CI for that).

- LEAN integration (NuGet + thin launcher, C#/Python e2e); KRX-correct stats (Asia/Seoul TZ + constant risk-free rate)
- **Docker (권장 설치·Windows 유일 경로)** — `Dockerfile`(base `python:3.11-slim` + .NET 10 SDK 레이어; pythonnet이 **정확히 3.11** 필요해 dotnet SDK 이미지 대신 python 베이스) + `docker-compose.yml` + `.dockerignore`. 이미지 빌드 때 런처·KIS 어댑터 DLL·NuGet복원(+AlgorithmImports)·`.leanpy`(pandas/numpy 3.11 venv)를 **미리 구워** 첫 백테스트 즉시 시작. **호스트 바인딩**: 서버 기본은 `127.0.0.1`(로컬전용, 매매제어 보유)이라 컨테이너에선 포트매핑이 안 닿음 → 이미지가 `BUYLOW_DASHBOARD_HOST=0.0.0.0`(신설 `config.get_dashboard_host`, env→config→기본 127.0.0.1) 설정, **로컬전용은 compose가 호스트쪽을 `127.0.0.1:<port>`로만 매핑해 보존**(외부노출X). **영속은 디렉토리 마운트만**(`data/`·`runs/`·`state/`) → 사전준비(cp/touch) 불필요·OS무관(Windows 포함). ⚠️ **개별 '파일' bind-mount는 없으면 Docker가 디렉토리로 만드는 footgun**이 있어, 런타임 상태 3개 파일 경로를 env로 옮겨(`BUYLOW_CONFIG_LOCAL`=config.py·`BUYLOW_DB_PATH`=store.default_db_path·`BUYLOW_KIS_TOKEN_CACHE`=brokers/kis.py; 기본은 repo루트, 네이티브 불변) 이미지가 전부 `/app/state`로 모음 → compose가 `./state` 한 디렉토리만 마운트. 키는 실행 후 대시보드 설정탭 입력(`state/config.local.yaml` 저장). `BUYLOW_PORT`로 호스트포트 변경. 시크릿/대용량은 `.dockerignore`로 이미지에 안 구움
- **Cross-platform env resolution** (`orchestrator/lean/environment.py`) — backtest·live 공통 런타임 해석을 macOS/Linux **+ Windows**로 일반화. OS별 분기: libpython 파일명·위치(`python311.dll`은 인터프리터 설치 루트, Unix는 `sysconfig LIBDIR`), Python 3.11 탐색(`_find_python311` — Windows `py -3.11`/`python`, Unix `python3.11`; argv 리스트라 2토큰 지원·버전검증), venv 레이아웃(`Scripts\python.exe` vs `bin/python`), site-packages(`sysconfig.get_path('purelib')` — Windows에서 `site.getsitepackages()[0]`는 venv 루트라 오답), dotnet(`dotnet.exe`). 순수 분기는 `tests/test_environment.py` 단위테스트. ⚠️ **Windows 분기 코드·테스트는 남겨두되, Windows 네이티브 설치는 더 이상 문서화 경로가 아님** — Windows는 Docker로 안내(런타임 손맞춤 회피). 문서화된 네이티브 경로는 Linux·macOS. 이전엔 `python3.11` 미탐색→`brew` 안내로 오인됐고 Windows는 명시적 미지원이었음
...
- **Toss live adapter** (`adapter/MyTrading.Toss`, `MyTrading.Toss.dll`) — C# `TossBrokerage` (+`IDataQueueHandler`): `TossRestClient` (OAuth2 client-credentials 토큰캐시, `getAccounts`로 accountSeq 자동해석, holdings/buying-power/market-calendar, createOrder/modify/cancel, **getOrder 폴링**), `TossSymbolMapper`, `TossBrokerageModel`(`KoreanFeeModel` 복제 — KIS와 동일 비용), `TossBrokerageFactory`. ⚠️ **KIS와 핵심 차이 — 토스는 실시간 웹소켓이 없다**: 체결은 PlaceOrder가 받은 orderId를 추적해 **백그라운드 폴러(getOrder ~1.5s)**가 누적 체결수량 증분만큼 OrderEvent를 발생(부분→PartiallyFilled, 전량→Filled, 취소/거부/만료→Canceled). 시세는 **현재가 폴러(getPrices ≤200종목 ~2s)**로 구독종목을 데이터피드에 흘림. 그래서 **HTS ID 불필요**. 계좌 지정은 `X-Tossinvest-Account` 헤더(accountSeq). 모의 서버 없음(실전 단일). 주문은 KIS와 동일한 페이싱(250ms)+백오프 재시도+`Ok=false` 비치명 정책. `live_start_ok`(toss)=`enabled`+client_id/secret. Live config는 `runner.build_toss_live_config`(`live-toss` env, `live-mode-brokerage="TossBrokerage"`); `runner.run_live`가 `config.get_broker()`로 KIS/토스 어댑터·env를 분기(`LIVE_ADAPTERS`). Tests: `tests/test_toss.py`(클라이언트 파싱·브로커·라이브 config), `adapter/MyTrading.Toss.Tests`(응답 파싱·주문 상태 분류). See **[docs/LIVE_TOSS.md](./docs/LIVE_TOSS.md)**.
...
- ⚠️ **Toss real-order e2e** — Toss는 모의 서버가 없어 실계좌로만 검증 가능(소액·소수종목 권장; 절차 LIVE_TOSS.md). 어댑터·배선·폴링 체결/시세는 구현·단위테스트 완료. 남은 한계: 폴링 지연(체결 ~1.5s/시세 ~2s), open-order resync 미구현(보유는 잔고로 실측), 부분체결 평균단가 근사, 매매내역은 자체 거래로그 폴백(Toss CLOSED 조회 미지원).
