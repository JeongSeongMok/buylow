...
- **Resolution-driven config (전략 설정 UI)** — 리스크 관리 + 체결을 하나의 카드로 합치고 **해상도(일봉/분봉)가 나머지를 결정**한다(사용자가 일일이 안 고르게; `execution_from_form` derives select_eval/risk_eval from resolution):
  ...
  - **분봉**: 선별=장중 매분(`select_eval=intraday`), 리스크 평가=매분(`risk_eval=bar`), 체결 방식=눌림목/TWAP(`style` ∈ pullback|twap; immediate는 사용자 선택에서 제외 — 폴백 전용). 분봉 있으면 그 전략, **없는 종목/일은 일봉(시가)로 자동 폴백**(`TimingConfig.for_availability`, `lean_format.list_minute_days`)
